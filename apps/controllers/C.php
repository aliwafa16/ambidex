...
class C extends CI_Controller
{
    ...
    public function index($alias)
    {
        ...
        if ($search_account) {
            ...
            if ($event_account) {
                ...
                $data['pendidikan'] = $this->db->get('table_pendidikan')->result_array();
                ...
                // Get item pertanyaan
                // $this->db->where('f_variabel_id !=', 5);
                $itemPernyataan = $this->db->get('t_item_pernyataan')->result_array();
                $groupedData = array_chunk($itemPernyataan, 11);


                // // Get item mood
                // $this->db->where('f_variabel_id', 5);
                // $itemMood = $this->db->get('t_item_pernyataan')->result_array();


                $max_group = 10;
                $data['item_pernyataan'] = $groupedData;
                // $data['item_mood'] = $itemMood;
    ...
    function fnSaveFinish($id, $aliasUrl)
    {
        ...
        if (!$event_account) {
            ...
            // Hitung exploitation & exploration
            $rowTipe = [
                [
                    'nama' => 'exploitation',
                    'value' => 0,
            ...
            foreach ($distribusi_jawaban as $jawaban) {
                if ($jawaban['tipe'] == 1) {
                    $rowTipe[0]['value'] += $jawaban['value'];
                    $rowTipe[0]['index'] = round($rowTipe[0]['value'] / 11, 2);
                    $rowTipe[0]['percentage'] = ($rowTipe[0]['index'] / 4) * 100;
                }

                if ($jawaban['tipe'] == 2) {
                    $rowTipe[1]['value'] += $jawaban['value'];
                    $rowTipe[1]['index'] = round($rowTipe[1]['value'] / 11, 2);
                    $rowTipe[1]['percentage'] = ($rowTipe[1]['index'] / 4) * 100;
                }
            }
            ...
            $array = array(
                ...
                'f_survey_username' => htmlspecialchars($inputData['username']), //$this->session->userdata('f_survey_username'),
                'f_survey_email' => htmlspecialchars($inputData['email']), //$this->session->userdata('f_survey_username'),
            ...
            $data_survey = array("distribusi_jawaban" => $distribusi_jawaban, "jawaban_perkategori" => $distribusi_perkategori, 'jawaban_tipe' => $rowTipe);
    ...
    public function finish($survey_password)
    {
        ...
        foreach ($data['survey']['jawaban_perkategori'] as $key => $value) {
            $label[] = $value['name'];
            $data_value[] = $value['value'];
        };

        $data['label_chart'] = $label;
        $data['data_value_chart'] = $data_value;
