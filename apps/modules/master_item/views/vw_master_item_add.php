		...
		<div class="card">
			...
			<div class="card-body">
				...
				<form name="frmMaster_item" id="frmMaster_item" method="post" class="form-horizontal">
					...
					<div class="form-group row">
						<label class="col-md-2" style="padding-top:7px;">Item Pernyataan (Eng) <span class="text-red">*</span></label>
						<div class="col-md-6"><textarea name="f_item_name_eng" id="f_item_name_eng" class="form-control" placeholder="Item Pernyataan ..." rows="5" required></textarea></div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-md-6">
							<?php
							$dataTes = array(1 => 'Positif', 2 => 'Negatif');
			...
			if (e.params.data.id) {
				$.getJSON('<?= site_url('master_item/fnComboDataDimensi'); ?>?var_id=' + e.params.data.id, function(e) {
					...
					if (size > 0) {
						...
						$("#f_dimensi_id").empty().select2({
							data: options
						});
					} else {
						swal.fire({
							text: 'Tidak ada Dimensi',
							icon: 'info',
		...
		<?php if ($fAct == 'Edit') {; ?>
			$.get("<?php echo site_url('master_item/fnMaster_itemDataId?id=' . $fId); ?>", function(data) {
				...
				if (data) {
					...
					$('#f_item_name_eng').val(data.f_item_name_eng);
					$('#f_variabel_id').val(data.f_variabel_id).trigger('change');
					var options = [];
					options.push({
						text: data.f_dimensi_name,
						id: data.f_dimensi_id
					})
					$("#f_dimensi_id").empty().select2({
						data: options
					});
					...
					$.each(data.f_item_array, function(a, b) {
						...
						var kete = "<input type=\"text\" name=\"keterangan[]\" id=\"keterangan\" class=\"form-control ket" + no + "\" placeholder=\"Keterangan ...\" value=\"" + b.label + "\"  required>";
