...
class Mo_master_item extends CI_Model
{
	...
	{
		$this->db->select('a.f_id,a.f_item,a.f_variabel_id,a.f_answer,a.type,c.f_variabel_name, a.f_item_eng');
		$this->db->join('t_variabel c', 'c.f_id=a.f_variabel_id');
		$sql = $this->db->get_where($this->table . ' a', array('a.f_id' => $id))->row_array();
		$data = array(
			'f_id' => $sql['f_id'],
			'f_item_name' => $sql['f_item'],
			'f_item_name_eng' => $sql['f_item_eng'],
			'f_variabel_id' => $sql['f_variabel_id'],
			'f_item_array' => json_decode($sql['f_answer'], true),
			'f_type' => $sql['type'],
			'f_variabel_name' => $sql['f_variabel_name'],
		);
	...
	function updateData($id)
	{
		$data = array(
			//'f_id' => $this->input->post('f_id'),
			'f_item' => $this->input->post('f_item_name'),
			'f_item_eng' => $this->input->post('f_item_name_eng'),
			'f_variabel_id' => $this->input->post('f_variabel_id'),
			// 'f_dimensi_id' => $this->input->post('f_dimensi_id'),
		...
		$data['f_answer'] = json_encode($pilihan);
